...
int mainproc(std::string internalstringauxreadx,std::string internalstringauxready,int order,int boxcarsizeint,float risingthreshold,int nruns,std::string spectrafilename,std::string peaksfilename){
											...
											for (j=0;j<peakscount;j++){
												ret=snprintf(chartemppeakbuffer,sizeof chartemppeakbuffer,"%f",peaks[j]);
...
void* procdatath(void* argument){
							...
							if(fastchararrbcounter>3){
								...
								int retmainprocstatus=mainproc(internalstringauxreadx,internalstringauxready,order,boxcarsizeint,risingthreshold,nruns,spectrafilename,peaksfilename);
