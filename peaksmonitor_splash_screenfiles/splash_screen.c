	...
	if(hhdddsgdha!=0){
		if(tempa[0]=='\2'){
			gtk_main_quit();
				...
				gdfkghdk=gdfkghdk+1;
				sprintf(tempimg,"../res/pl%c.svg",tempa[gdfkghdk-1]);
				if(upimg==0){
				gtk_image_set_from_file(GTK_IMAGE(imgb[gdfkghdk-3]),tempimg);}else{sprintf(tempimg,"../res/pl%c.svg",' ');gtk_image_set_from_file(GTK_IMAGE(imgb[gdfkghdk-3]),tempimg);}
			}
...
int main(int argc, char *argv[]){
	...
	if(argc<=1){
		...
		chdir(path.c_str());
		execl((path+"../peaks").c_str(),(path+"../peaks").c_str());
		return 0;
	}
	...
	gtk_builder_add_from_file(builder,"../res/pui.glade",NULL);
