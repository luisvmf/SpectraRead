...
def initgui(windowclose,datavar,datavaronlyone,currentshowlength):
	...
	def updatedata(datavar,datavaronlyone,currentdataarr,datasourcesupdateinforealname,lastupdatetime,graphtimearray,starttime,pointdeleteval,totaldataarr,currentpointtime):
	...
	GObject.timeout_add(1, updatedata,datavar,datavaronlyone,currentdataarr,datasourcesupdateinforealname,lastupdatetime,graphtimearray,starttime,pointdeleteval,totaldataarr,currentpointtime,priority=GObject.PRIORITY_LOW)
	...
	while(windowclose.value=="opened"):
		if(aaa<200):
			...
			if(aaa<300):
				...
				i=0
				while(i<4):
					a=a+str(i+random.random()+23*math.sin(j/20.0))+" "
					i=i+1
				a=a+str(i+random.random())
				a=a+"|spectraread:"
				j=j+1
