...
def initgui(windowclose,datavar,datavaronlyone,currentshowlength):
	...
	while(k<len(graphtimearray)):
		...
		k=k+1
	def desenhagrafico(i,autoscaleval,entrytextselectstyle,rect,mousepos,inside,recttext,mousepostext,winsize,timecounterb,infotext,returnedfalse,oldmouseposloopcontrol,selecteddatarows,linesshown,graphtimearray,currentshowlength):
		#-------------------------------
		#-------------------------------
		#-------------------------------
	...
	def loopcontrol(i,autoscaleval,entrytextselectstyle,rect,mousepos,inside,recttext,mousepostext,winsize,timecounterb,infotext,returnedfalse,oldmouseposloopcontrol,selecteddatarows,linesshown,graphtimearray,currentshowlength):
		...
		if(returnedfalse[0]==1):
			GObject.timeout_add(50, desenhagrafico,i,autoscaleval,entrytextselectstyle,rect,mousepos,inside,recttext,mousepostext,winsize,timecounterb,infotext,returnedfalse,oldmouseposloopcontrol,selecteddatarows,linesshown,graphtimearray,currentshowlength,priority=GObject.PRIORITY_LOW)
		return True
	infotext=""
	GObject.timeout_add(150, loopcontrol,i,autoscaleval,entrytextselectstyle,rect,mousepos,inside,recttext,mousepostext,winsize,timecounterb,infotext,returnedfalse,oldmouseposloopcontrol,selecteddatarows,linesshown,graphtimearray,currentshowlength,priority=GObject.PRIORITY_LOW)
	...
	def updatedata(datavar,datavaronlyone,currentdataarr,datasourcesupdateinforealname,lastupdatetime,graphtimearray,starttime):
		...
		for newdatavarb in newdatavara:
			if(newdatavarb!=""):
				...
				if(abs(lastupdatetime[0]-time.time())>0.01):
					#print cdataprimnames
					#print cdataprim
					graphtimearray.append(time.time()-starttime)
					del graphtimearray[0]
					contimearr.append(time.time()-starttime)
					i=0
					for v in cdataprimnames:
						...
						for vb in cdataprim[i]:
							...
							if(cval!=-1):
								...
								if(len(contarray[cval])>100):
									if((len(contarray[cval])%2)==0):
										contarray[cval]=(np.asarray(contarray[cval]).reshape(-1, 3).mean(axis=1)).tolist()
	...
	GObject.timeout_add(1, updatedata,datavar,datavaronlyone,currentdataarr,datasourcesupdateinforealname,lastupdatetime,graphtimearray,starttime,priority=GObject.PRIORITY_LOW)
	...
	class Handler:
		...
		def changesize(self,*args):
			...
			#print ""+str(datapointsentrywidget.get_value())
	windowc.hide()
	windowc.show_all()
	windowc.hide()
	helpwin.hide()
