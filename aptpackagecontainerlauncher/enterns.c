...
fd = open(argv[4], O_RDONLY | O_CLOEXEC);
           ...
           if (setns(fd, 0) == -1)       /* Join that namespace */
               err(EXIT_FAILURE, "setns");
//If using exec, spectraread fails to start, probabily because /opt/SpectraRead/spectraread closes killing all forked processes before forked processes can exec needed processes.
           system("/opt/SpectraRead/spectraread");
       }
