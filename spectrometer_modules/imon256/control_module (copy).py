import time
import random
from spectrareadapi import setxdata, setydata, seterr, setwarning, settemp, pushval
...
def readprocess(instid):
	print "a"
	mapb=fastmmap.connectmmap("spectrareadd","w"+str(instid))
	while(mapb==-1):
		mapb=fastmmap.connectmmap("spectrareadd","w"+str(instid))
	while(True):
		i=0
...
def load(instid):
	...
	p2.start()
	if(mapa==-1):
		mapa=fastmmap.connectmmap("None","w"+str(instid))
def unload(instid):
	global p2
	p2.terminate()
