				...
				while(durationtotalacquisition/1000000<nruns){
					...
					while(countrawstrconvert<rawreaddata.length()){
						if(rawreaddata[countrawstrconvert]!=';'){
							...
							fastchararrb[ijj]='\0';
							//printf("%c",fastchararr[0]);
							//std::cout<<fastchararrb[ijj-1]<<"\n";
							internalstringauxread=std::string(fastchararrb);
							//std::cout<<internalstringauxread<<"\n";
							//internalstringauxread=std::string(reinterpret_cast<char*>(fastchararrb), fastchararrcounter-3);
							//internalstringauxread=reinterpret_cast<char*>(fastchararr);
							//std::cout<<internalstringauxread<<"\n\n\n";
