...
int d=1;
float progresspos=0;
char *finalresultreplace=malloc(9900);
gint TimerCallback(){
	char str[9900]="";
	char straaa[9900]="";
	...
	char *tempa=straaa;
	char *readdata=straaa;
	readdata=readmmap(mapid,0);


	char *res=straaa;
	char *strreplace=straaa;
	char *varreplace=straaa;
...
int main(int argc, char *argv[]){
	...
	if(argc<=1){
		...
		chdir(path.c_str());
		execl((path+"../spectraread").c_str(),(path+"../spectraread").c_str());
		return 0;
	}
