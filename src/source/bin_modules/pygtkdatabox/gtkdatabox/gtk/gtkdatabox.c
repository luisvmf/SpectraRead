...
//This file has been modified. It is different from the original version found at https://github.com/erikd/gtkdatabox.
//The modification is to fix a bug that caused memory leaks when resizing the window containing the graph.
//Modification is on lines 105,415,1163,1168,2433. TODO: Check if the cairo_t *crintg is always initialized to NULL on line 415 or 2433 before been referenced on line 1163.
// Luis Victor Muller Fabris. 08/January/2020.
...
struct _GtkDataboxPrivate {
    ...
    gint old_width;
    gint old_height;
	cairo_t *crintg; //XXX:BUG FIX:Fix memory leak when resizing window. luisvmf.com. Lines 105,415,1163,1168,2433.
...
gtk_databox_init (GtkDatabox * box) {
	...
	priv->crintg=NULL;//XXX:BUG FIX:Fix memory leak when resizing window. luisvmf.com. Lines 105,415,1163,1168,2433.
...
gtk_databox_create_backing_surface(GtkDatabox * box) {
   ...
   priv->old_height = height;
	//XXX:BUG FIX:Fix memory leak when resizing window. luisvmf.com. Lines 105,415,1163,1168,2433.
	if(!priv->crintg){
		priv->crintg = pgdk_cairo_create (gtk_widget_get_window (widget));
	}
   priv->backing_surface = cairo_surface_create_similar(
                                cairo_get_target (priv->crintg),//XXX:BUG FIX:Fix memory leak when resizing window. luisvmf.com. Lines 105,415,1163,1168,2433.
                                CAIRO_CONTENT_COLOR,
                                width, height);
