...
static PyObject* pyinitmmap_create(PyObject* self,  PyObject *args) {
	...
	return Py_BuildValue("i", fastmmapmq_createmmap(s,perm));
}
...
void initfastmmap(void) {
	Py_InitModule3("fastmmap", mmap_funcs,"Fast Message queue module, witch can pass messages between different processes, for Python, nodejs and c.");
}
