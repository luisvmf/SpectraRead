...
#define POINTS 9999 //This is the maximum number of points that can be shown in the graph.
...
static void create_graph (GtkWidget *box1,float *linecolor, float *pointcolor,float *textcolor,int darktheme){
   ...
   graphpeaks = gtk_databox_points_new (POINTS, Xpeaks, Ypeaks, &color, 9);
	gtk_databox_graph_add (GTK_DATABOX (box), graphpeaks);//para remover gtk_databox_graph_remove(box, graphpeaks); ou 	esconder gtk_databox_graph_set_hide(graphpeaks,TRUE);
