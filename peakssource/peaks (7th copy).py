...
def initgui(windowclose,datavar,datavaronlyone,currentshowlength):
					...
					if(1==1):
						...
						while(j<len(linearr)):
							if(bufferlength==len(currentdataarr[j])):
								...
								linearr[j-1].set_data((np.asarray(realtimestampfrompproc1[j-1][-currentshowlength.value:])-realtimestampfrompproc1[j-1][(len(realtimestampfrompproc1[j-1])-1)]),np.asarray(currentdataarr[j-1][-currentshowlength.value:]))
	...
	def loopcontrol(i,autoscaleval,entrytextselectstyle,rect,mousepos,inside,recttext,mousepostext,winsize,timecounterb,infotext,returnedfalse,oldmouseposloopcontrol,selecteddatarows,linesshown,graphtimearray,currentshowlength,datamodeselected,realtimestampfrompproc1):
		...
		return True
	infotext=""
	GObject.timeout_add(150, loopcontrol,i,autoscaleval,entrytextselectstyle,rect,mousepos,inside,recttext,mousepostext,winsize,timecounterb,infotext,returnedfalse,oldmouseposloopcontrol,selecteddatarows,linesshown,graphtimearray,currentshowlength,datamodeselected,realtimestampfrompproc1,priority=GObject.PRIORITY_LOW)
