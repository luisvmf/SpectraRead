...
def initgui(windowclose,datavar,datavaronlyone,currentshowlength):
	...
	while(k<len(graphtimearray)):
		...
		k=k+1
	def desenhagrafico(i,autoscaleval,entrytextselectstyle,rect,mousepos,inside,recttext,mousepostext,winsize,timecounterb,infotext,returnedfalse,oldmouseposloopcontrol,selecteddatarows,linesshown,graphtimearray,currentshowlength,datamodeselected):
		#-------------------------------
		#-------------------------------
		#-------------------------------
					...
					if(1==1):
						...
						while(j<len(linearr)):
							if(bufferlength==len(currentdataarr[j])):
								...
								linearr[j-1].set_data(np.arange(currentshowlength.value),np.asarray(currentdataarr[j-1][-currentshowlength.value:]))
	...
	def loopcontrol(i,autoscaleval,entrytextselectstyle,rect,mousepos,inside,recttext,mousepostext,winsize,timecounterb,infotext,returnedfalse,oldmouseposloopcontrol,selecteddatarows,linesshown,graphtimearray,currentshowlength,datamodeselected):
		...
		if(returnedfalse[0]==1):
			GObject.timeout_add(50, desenhagrafico,i,autoscaleval,entrytextselectstyle,rect,mousepos,inside,recttext,mousepostext,winsize,timecounterb,infotext,returnedfalse,oldmouseposloopcontrol,selecteddatarows,linesshown,graphtimearray,currentshowlength,datamodeselected,priority=GObject.PRIORITY_LOW)
		return True
	infotext=""
	GObject.timeout_add(150, loopcontrol,i,autoscaleval,entrytextselectstyle,rect,mousepos,inside,recttext,mousepostext,winsize,timecounterb,infotext,returnedfalse,oldmouseposloopcontrol,selecteddatarows,linesshown,graphtimearray,currentshowlength,datamodeselected,priority=GObject.PRIORITY_LOW)
	...
	def updatedata(datavar,datavaronlyone,currentdataarr,datasourcesupdateinforealname,lastupdatetime,graphtimearray,starttime,pointdeleteval,totaldataarr,currentpointtime,dataupdaterunning):
		if(dataupdaterunning[0]==1):
			return True
		dataupdaterunning[0]=1
		...
		for newdatavarb in newdatavara:
			if(newdatavarb!=""):
				...
				#print cdataprimnames
				#print datasourcesupdateinforealname
				if(abs(lastupdatetime[0]-time.time())>0.01):
					#print cdataprimnames
					#print cdataprim
	...
	GObject.timeout_add(1, updatedata,datavar,datavaronlyone,currentdataarr,datasourcesupdateinforealname,lastupdatetime,graphtimearray,starttime,pointdeleteval,totaldataarr,currentpointtime,dataupdaterunning,priority=GObject.PRIORITY_LOW)
...
def mainuiload(processid):
	...
	datavar = managerb.Value(c_char_p, "|qimon:1 2 3 4 5 6|spectraread:7 8 9,")
